...
export default function OverseerQueueScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    const subscription = QueueService.subscribeToQueueUpdates('overseer', (payload) => {
  ...
  const loadQueueCount = async () => {
    try {
      const count = await QueueService.getQueueCount('overseer');
          ...
          <View style={styles.body}>
            ...
            <View style={styles.headerCard}>
              <View style={styles.headerIconContainer}>
                <MaterialCommunityIcons name="account-tie" size={40} color="#8B1538" />
              </View>
